...
module MAP-{key.Name()}-TO-{value.Name()}-SYMBOLIC
  imports MAP-{key.Name()}-TO-{value.Name()}-KORE-SYMBOLIC
endmodule

module MAP-{key.Name()}-TO-{value.Name()}-KORE-SYMBOLIC  [symbolic]
  ...
  rule K in_keys((K {map_short}|-> _V) M:{map_name})
      => true
    requires notBool K in_keys(M)
    [simplification(50), preserves-definedness]
  rule K1 in_keys((K2 {map_short}|-> _V) M:{map_name})
      => K1 in_keys(M)
    requires true
